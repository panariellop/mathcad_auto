        ...
        if event == "OK" or event == sg.WIN_CLOSED:
            ...
            if event == "excel_name":
                if values["excel_name"] != "":
                    inputs = set_inputs_from_xl(values['excel_name'])
...
def set_inputs_from_xl(filepath: str)->dict:
    """
    ...
    sheet = wb['values']
    inputs['eqpt_name'] = sheet['A2'].value
    inputs['mounting_location'] = sheet['B2'].value.upper()
